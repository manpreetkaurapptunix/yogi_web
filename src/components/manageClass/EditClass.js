...
const EditClass = (props) => {
  ...
  useEffect(() => {
    if (instClassByIDData) {
      setImage(instClassByIDData?.image);
      setclassCategory(instClassByIDData?.categoryId?._id);
      setstartDate(new Date(instClassByIDData?.start_date).toString());
      setendDate(new Date(instClassByIDData?.end_date).toString());
      settimeArray(instClassByIDData?.time);
    }
  }, [instClassByIDData]);
  ...
  const formik = useFormik({
    ...
    initialValues: {
      totalSeats: instClassByIDData?.seats || "",
      pricePerHead: instClassByIDData?.price_per_head || "",
      className: instClassByIDData?.name || "",
      classDesc: instClassByIDData?.desc || "",
      cancellation: instClassByIDData?.cancel_policy || "",
    },
    ...
    onSubmit: async () => {
      ...
      if (
        ...
        if (!startTime && !endTime && timeArray?.length == 0) {
          return;
        }
      } else {
        isDelete && startTime && endTime;
        timeArray.push({
          start_time: new Date(startTime).toISOString(),
          end_time: new Date(endTime).toISOString(),
        });
  ...
  console.log(image, "jknjnkjn");

  return (
    <Box className="ad_cls_mn">
      <Box className="new_cls hdg_rw">
        <h3 align="center">Edit Class</h3>
        <h5
        ...
        <form className="form_input" onSubmit={formik.handleSubmit}>
          <Box className="input_field">
            {/* <Box className="upload_class">
              <Box className="img_upload_cls full_img p-0">
                <Image
                  width={"200px"}
                  height={"200px"}
          ...
          {isDelete && (
            <Box className="input_field dt_mn">
              <Box className="strt_dt">
                <Typography className="fw-bold" sx={{ marginBottom: "5px" }}>
                  Start Time
                </Typography>
                ...
                <Box className="time_bx">
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <TimePicker
                      ...
                      onChange={(newValue) => {
                        if (newValue.diff(startTime, "hours") >= 1) {
                          if (timeArray.length) {
                            timeArray.forEach((ele) => {
                              if (
                                dayjs(newValue).isBetween(
                                ...
                                toast.error(
                                  "You already select this time slot"
                                );
                                return;
                              } else {
                                count = count + 1;
